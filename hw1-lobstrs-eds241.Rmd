...
**Assignment submission (YOUR NAME):** Tom Gibbens-Matsuyama
...
From a Poisson model, we see different coefficients for our intercept and treat. We have an intercept of 3.12, meaning a value of 3.12 lobster counts when treat is equal to 0 or non-MPAs are present. We have a coefficient of 0.21 for treat, meaning we have an increase of 0.21 lobsters when MPAs are present.
...
The coefficients for this model are the same as the previous one. The treat coefficient can be interpreted the same as the previous model, indicating that lobster counts increases with a value of 0.21 when MPAs are present. Our standard error values are about 6 times greater than the previous model. Also, the z values are much smaller.
...
There is evidence of the treatment effect being robust and stable across the different models. There is robustness from our OLS model compared to the others as there is a significant change in our treat coefficient. Our OLS model has a treat coefficient of 5.36 while our Poisson and NB model have a treat coefficient of 0.21. That being said, there is also stability in the treat coefficient when we jump from a Poisson to NB model. Stated earlier, the treat coefficient stays the same.
